...
class TestNoteSuccess:
    ...
    def test_note_create_by_2user(self, client, generate_usertoken2):
        ...
        url = reverse('note-list')
        response = client.post(
            url, HTTP_AUTHORIZATION=f'Bearer {generate_usertoken2}', data=data, content_type='application/json')
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_add_collaborators(self, client, generate_usertoken, generate_usertoken2):
        ...
        data = {
            "note_id": note_id,
            "user_ids": [User.objects.get(email="sonalraj2002@gmail.com").id] # type: ignore
        }
    ...
    def test_remove_collaborators(self, client, generate_usertoken, generate_usertoken2):
        note_id = self.test_note_create(client, generate_usertoken)
        collaborator_res = User.objects.get(email="sonalraj2002@gmail.com")
        ...
        # assert "Collaborators removed successfully" in response.data['message']
# 2024-09-05T16:17:44.131002+0530 - INFO - 11----------?sonalraj2002@gmail.com????>>>>>{'message': 'No collaborators were removed. The provided user IDs may not be collaborators on this note.', 'status': 'error'}
    ...
    def test_delete_note_by_different_user(self, client, generate_usertoken, generate_usertoken2):
        ...
        url = reverse('note-detail', args=[note_id])
        response = client.delete(
            url, HTTP_AUTHORIZATION=f'Bearer {generate_usertoken2}', content_type='application/json')
        assert response.status_code == status.HTTP_404_NOT_FOUND
